...
// === Database: AppDbContext wired to SQL Server via "DefaultConnection" in appsettings.json
builder.Services.AddDbContext<AppDbContext>(options => options.UseSqlServer(builder.Configuration.GetConnectionString("DefaultConnection")));

// === Identity: AppUser + roles with relaxed password rules; stored via EF Core
...
// === CORS: Allow Angular dev app on localhost:4200 (any header/method)
...
// === DB migration & seed initial Industries if empty
